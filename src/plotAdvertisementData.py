...
if __name__ == "__main__":
    ...
    advertisement_path = os.path.join(file_path, "data", "optimizely_advertisement.tsv")
    ...
    # read and plot
    df = pd.read_csv(advertisement_path, sep = "\t")
    ...
    # type distribution
    tdf = df.drop_duplicates(subset = ["domain", "advertisement"])
    tdf = tdf[tdf["advertisement"] != "amazonaws"]
    ...
    ax.set_yticklabels(["Adition", "Celtra", "Inspectlet", "254A", "FairFax", "Clickable", "TRUSTe", "Adform", "Outbrain", "CrazyEgg", "Demandbase", "Mixpanel", "Clicktale"])
